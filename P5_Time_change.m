%2D Heat Equation.
clear; close all; clc
n = 20;  % grid has 10 points per dimension (overlapping)
...
T(n/2,1:n) = exp(-1*1*(x+n/4).^2) + exp(-1*1*(x-n/4).^2); % Middle depth
T(n,1:n) = 0;  %Top
% T(1:n,n/4) = 0.0001*exp(-0.1*(y+n/2).^2);  %LEFT
% T(1:n,n-n/4) = 0.0001*exp(-0.1*(y+n/2).^2);  %RIGHT
D=1;
dt = dx^2/8*D; % delta t
error = 1; 
k = 0;
...
v = VideoWriter('Two Landmines.mp4','MPEG-4');
